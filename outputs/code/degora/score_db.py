...
QUALITY_WEIGHTED_SCORE_FORMULA = (
    "Exploratory secondary ranking only: same component formula as the primary "
    "DEGORA score, but source-unit evidence is weighted by predeclared source "
    "quality features and a gold-panel-free source-coherence guardrail. It does "
    "not replace degora_score and is not a calibrated probability."
)
...
def _source_unit_series(frame: pd.DataFrame) -> pd.Series:
    paper_id = _string_column(frame, "paper_id")
    study_id = _string_column(frame, "study_id")
    source_unit = paper_id.str.strip()
    source_unit = source_unit.mask(source_unit.eq(""), study_id.str.strip())
    return source_unit.fillna(study_id)
...
def _quality_weighted_consensus(
    ...
    *,
    ...
    grouped["quality_weighted_consensus_direction"] = np.select(
        [grouped["quality_weighted_lfc"].gt(0), grouped["quality_weighted_lfc"].lt(0)],
        ["up", "down"],
        default="flat",
    )
...
def _direction_confidence_from_evidence(
    ...
    *,
    ...
    frame["_wz"] = frame["_unit_weight"] * frame["signed_z"]
    combined = frame.groupby("gene_symbol", as_index=False).agg(combined_z=("_wz", "sum"))
    signed = frame.merge(combined, on="gene_symbol", how="left")
    signed["_combined_sign"] = np.sign(signed["combined_z"])
    signed["_concordant"] = np.where(
        signed["_combined_sign"].ne(0),
        np.sign(signed["signed_z"]).eq(signed["_combined_sign"]),
        False,
    )
    signed["_success_weight"] = np.where(signed["_concordant"], signed["_unit_weight"], 0.0)
...
def _leave_one_source_out_stability(
    ...
    *,
    ...
    if evidence.empty or scores.empty:
        return pd.DataFrame(columns=columns)
    source_units = sorted(evidence["source_unit_id"].dropna().astype(str).unique())
    ...
    for source_unit in source_units:
        subset = evidence.loc[evidence["source_unit_id"].astype(str).ne(source_unit)].copy()
    ...
    *,
    ...
    if consensus.empty:
        metadata = {
            ...
            "direction_confidence_rule": "Beta(1,1)-shrunk source-unit count concordance: (1 + concordant source units) / (2 + observed source units); quality-weighted direction confidence uses reliability-weighted pseudo-counts and is not a calibrated posterior probability",
    ...
    raw_support["gene_symbol"] = _string_column(raw_support, "gene_symbol").str.upper().str.strip()
    raw_support["study_id"] = _string_column(raw_support, "study_id").str.strip()
    raw_support["source_unit_id"] = _source_unit_series(raw_support)
    support = raw_support.dropna(subset=["gene_symbol", "study_id", "source_unit_id"]).groupby("gene_symbol", as_index=False).agg(
        n_contrasts_observed=("study_id", "nunique"),
        n_source_units=("source_unit_id", "nunique"),
    ...
    direction_confidence = _direction_confidence_from_evidence(evidence, output_column="direction_confidence_index")
    ...
    scores["consensus_direction"] = np.select(
        [scores["weighted_lfc"].gt(0), scores["weighted_lfc"].lt(0)],
        ["up", "down"],
        default="flat",
    )
    ...
    metadata = {
        ...
        "n_contrasts_total": int(raw_support["study_id"].nunique()) if not raw_support.empty else 0,
        "n_nonfinite_lfc_capped_for_score": n_nonfinite_lfc_capped,
        "independent_unit_for_consensus": "source_unit_id (paper_id when available, otherwise study_id)",
        "source_unit_collapse_rule": SOURCE_UNIT_COLLAPSE_RULE,
        "direction_concordance_rule": "evidence-strength-weighted concordance across independent source-unit representatives",
