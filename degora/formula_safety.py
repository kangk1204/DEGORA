...
def contains_formula_guard(frame: pd.DataFrame) -> bool:
...
def restore_formula_text_if_marked(frame: pd.DataFrame, path: str | Path) -> pd.DataFrame:
    ...
    guarded = contains_formula_guard(frame)
    if state == "invalid" and guarded:
        raise ValueError(
            f"{Path(path).name} has invalid formula-guard provenance: its marker, digest, "
            "or provenance JSON does not match the artifact"
        )
    if guarded:
        raise ValueError(
            f"{Path(path).name} contains apostrophe-guarded formula-like text but has no "
