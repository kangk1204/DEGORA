...
DEFAULT_DEMO_SEARCH_KEYWORD = "hypoxia normoxia renal epithelial"
DEFAULT_DEMO_SEARCH_SPECIES = "human"
    ...
    *,
    ...
    return pd.DataFrame(
        [
            ...
            {
                "field": "demo_search_keyword",
                "value": search_keyword,
                "what_to_enter": "Suggested live GEO discovery keyword; the dashboard pre-fills but does not submit it.",
            },
            {
                "field": "demo_search_species",
                "value": search_species,
                "what_to_enter": "Suggested live discovery tab: human or mouse.",
            },
        ]
    )
...
def _readme_rows(*, search_keyword: str, search_species: str) -> pd.DataFrame:
    return pd.DataFrame(
        [
            ...
            {
                "step": 6,
                "instruction": f'Open Discover studies; {search_species.title()} is suggested with keyword "{search_keyword}".',
            },
            {"step": 7, "instruction": "Press Search GEO yourself; search-time file badges are likelihoods, not confirmed inputs."},
        ]
    )
...
def _advanced_rows() -> pd.DataFrame:
    return pd.DataFrame(
        [
            {"setting": "score_version", "value": SCORE_VERSION, "what_it_does": "Fixed prioritization-score specification."},
...
def write_demo_workspace(
    path: str | Path,
    *,
    force: bool = False,
    search_keyword: str | None = None,
    search_species: str | None = None,
) -> dict[str, str]:
    """Write a complete synthetic demo workspace with source DEG tables and Excel config."""

    from .discovery import build_geo_query, normalize_species

    clean_keyword = " ".join((search_keyword or DEFAULT_DEMO_SEARCH_KEYWORD).split())
    species = normalize_species(search_species or DEFAULT_DEMO_SEARCH_SPECIES).key
    ...
    with pd.ExcelWriter(config_path, engine="openpyxl") as writer:
        write_note_sheet(writer, "README", _readme_rows(search_keyword=clean_keyword, search_species=species))
        write_note_sheet(
            writer,
            "Project",
            _project_rows("results", "harmonized", search_keyword=clean_keyword, search_species=species),
        )
    ...
    readme_path.write_text(
        "\n".join(
            [
                ...
                "",
                "Live GEO discovery (internet required):",
                "",
                f'- Open Discover studies; the {species.title()} tab is suggested with keyword `{clean_keyword}`.',
                "- Press Search GEO yourself. DEG-input badges are filename-based likelihoods, not confirmation.",
                "- Prepare selected studies to download and validate candidate tables before analysis.",
                "",
            ]
        )
    )
