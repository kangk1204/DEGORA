...
@pytest.mark.parametrize("no_excel", [False, True])
def test_run_warns_when_gold_panel_cannot_support_recall_metrics(tmp_path, capsys, no_excel) -> None:
    source_a = tmp_path / "source_a.csv"
    source_b = tmp_path / "source_b.csv"
    _write_source(source_a, ["ISG15", "IFIT1", "RPL13A"], 1.0)
    _write_source(source_b, ["ISG15", "IFIT1", "RPL13A"], 0.8)
    ...
    assert message.count("GoldPanel is missing the required gene_symbol column") == 1
    metrics = json.loads((tmp_path / "results" / "slice_metrics.json").read_text())
    assert metrics["gold_panel_status"] == "invalid"
    if not no_excel:
        manifest = json.loads((tmp_path / "results" / "DEGORA_output.manifest.json").read_text())
        assert manifest["gold_panel"]["status"] == "invalid"


def test_validate_warns_when_gold_panel_cannot_support_recall_metrics(tmp_path, capsys) -> None:
    source_a = tmp_path / "source_a.csv"
    source_b = tmp_path / "source_b.csv"
    _write_source(source_a, ["ISG15", "IFIT1", "RPL13A"], 1.0)
    _write_source(source_b, ["ISG15", "IFIT1", "RPL13A"], 0.8)
